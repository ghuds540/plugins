...
def validate_scraper(client: StashInterface, scraper_id: str) -> str:
    ...
    # Debug: Log all available scrapers
    log.debug(f"Looking for scraper with ID: '{scraper_id}'")
    log.debug(f"Available image scrapers: {[s.get('id', 'NO_ID') for s in scrapers]}")
    for scraper in scrapers:
        log.debug(f"Scraper details: {scraper}")
